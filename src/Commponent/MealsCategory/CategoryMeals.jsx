...
export default function CategoryMeals() {
  ...
  let [CateMeal, SetCateMeal] = useState([])
  let Param = useParams()

  async function getCategoryMeals() {
    let { data } = await axios.get(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${Param.mealsCat}`)
    SetCateMeal(data.meals);
  }
  useEffect(() => {
    getCategoryMeals()
  }, [])
  ...
  return (
    <div className='row py-5 gy-3 gx-3'>
      {CateMeal.length ? CateMeal.map((mael, i) => (
